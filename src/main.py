...
# meta-llama/Llama-3.1-8B-Instruct — Max tokens: TBD (likely 8k–16k), Max characters: ~25,000,000–50,000
# mistralai/Mixtral-8x7B-Instruct-v0.1 — Max tokens: 32768, Max characters: ~100,000–130,000


# # ======= Model import =======
login("PUT HERE YOUR HUGGING FACE TOKEN")
...
max_chars_total = 512000 # See max characters per model above
# # ===================================
...
def run_model(model_name, people_list, rag, filtered_output=None):
    ...
    for person in progress:
        ...
        status_str = "✅ Member" if member_status else "❌ Not a Member"
        tqdm.write(f"{full_name}: {status_str}")
        sys.stdout.flush()  # 👈 Force flushing output to keep each line separate
...
dataset_type = "switzerland_nw"  # "international", "switzerland" or switzerland_nw"
